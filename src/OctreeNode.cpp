#include "fast_simulator/OctreeNode.h"
...
void OctreeNode::add(const tf::Vector3& p) {
    ...
    children_[index]->add(tf::Vector3(x, y, z));
}

void OctreeNode::getCubes(std::vector<Box>& cubes, const tf::Vector3& offset) const {
    if (occupied_) {
        cubes.push_back(Box(offset, offset + tf::Vector3(size_, size_, size_)));
    } else {
        int i = 0;
        for(int x = 0; x <= 1; ++x) {
            for(int y = 0; y <= 1; ++y) {
                for(int z = 0; z <= 1; ++z) {
                    if (children_[i]) {
                        children_[i]->getCubes(cubes, offset + tf::Vector3(split_ * x, split_ * y, split_ * z));
                    }
                    ++i;
                }
            }
...
bool OctreeNode::intersect(const Ray& r, float t0, float t1, double& distance, const tf::Vector3& offset) const {
    ...
    tf::Vector3 o = r.origin + t0 * r.direction;
    ...
    if (children_[index]) {
        if (children_[index]->intersect(Ray(r.origin - tf::Vector3(dx, dy, dz), r.direction), t0, t1, distance, offset + tf::Vector3(dx, dy, dz))) {
            return true;
        }
    ...
    double dist;
    Box b(tf::Vector3(dx, dy, dz), tf::Vector3(dx + size_ / 2, dy + size_ / 2, dz + size_ / 2));
    b.intersect(Ray(o, -r.direction), 0, t1 - t0, dist);
    distance = t0 - dist;
    return this->intersect(r, distance + tree_->resolution_ * 0.1, t1, distance, offset);
}

void OctreeNode::raytrace(const tf::Vector3& o, const tf::Vector3& dir, float t0, float t1, const tf::Vector3& offset) {
    if (t1 < t0) {
        return;
    }

    tf::Vector3 newo = o + t0 * dir;
    ...
    if (children_[index]) {
        children_[index]->raytrace(o - tf::Vector3(dx, dy, dz), dir, t0, t1, offset + tf::Vector3(dx, dy, dz));
    }
...
bool OctreeNode::intersect(const tf::Vector3& p) const {
    if (occupied_) {
        return true;
    }
    ...
    for(int x = sx; x <= ex; ++x) {
        for(int y = sy; y <= ey; ++y) {
            for(int z = sz; z <= ez; ++z) {
                int i = 4 * x + 2 * y + z;
                if (children_[i]) {
                    tf::Vector3 offset(split_ * x, split_ * y, split_ * z);
